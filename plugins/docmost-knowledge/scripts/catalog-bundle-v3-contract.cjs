...
const VERIFIED_PAGE_CACHE = new Map();
const MAX_VERIFIED_PAGE_CACHE_ITEMS = 2_048;
...
function validateBundleV3(args, bundle, options = {}) {
  ...
  cacheVerifiedPages(pages, options.verifiedPageCache);
  return bundle;
}
...
function validateDeltaV3(args, delta, options = {}) {
  ...
  const currentPages = reconstructDeltaPages(
    delta.changes,
    options.verifiedPageCache,
  );
  validateCatalogContent(
    {
      pages: currentPages,
...
function requireTrustedPublicKeys(trustedPublicKeys) {
  if (
    ...
    Object.keys(trustedPublicKeys).length === 0
  ) {
    fail("Catalog v2/v3 requires profile catalogPublicKeys pins");
  }
}

function cacheVerifiedPages(pages, cache = VERIFIED_PAGE_CACHE) {
  for (const page of pages) {
    const key = verifiedPageCacheKey(page);
    if (cache.has(key)) cache.delete(key);
    cache.set(key, page);
  }
  while (cache.size > MAX_VERIFIED_PAGE_CACHE_ITEMS) {
    cache.delete(cache.keys().next().value);
  }
}

function reconstructDeltaPages(changes, cache = VERIFIED_PAGE_CACHE) {
  const pages = [
    ...changes.added,
    ...changes.updated.map((item) => item.current),
  ];
  for (const manifest of changes.unchanged) {
    const page = cache.get(verifiedPageCacheKey(manifest));
    if (!page) {
      fail(
        "Catalog Delta static page cache is incomplete; resolve a fresh Bundle before Delta",
      );
    }
    pages.push(page);
  }
  return pages.sort((left, right) => compare(left.page_id, right.page_id));
}
